...
class Middleware
{
    ...
    private function runMiddlewares(string $httpMethod, string $uri, Request $request): Request
    {
        ...
        $routeMiddleware = app()->ps->get_app_conf()['route_middleware'];
    ...
    {
        //Получаем список всех разрешенных классов middlewares из настроек приложения
        $routeMiddleware = app()->ps->get_app_conf()['route_app_middleware'];
    ...
    public function runMiddlewaresOLD(string $httpMethod, string $uri): Request
    {
        $request = new Request();
        //Получаем список всех разрешенных классов middlewares из настроек приложения
        $routeMiddleware = app()->ps->get_app_conf()['route_middleware'];
